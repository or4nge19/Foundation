...
Using `.carrier` extracts the raw `Set W` explicitly, bypassing typeclass inference gaps.
-/
def Forces {α W : Type u}[Preorder W] (Norm : UpperSet W) (V : α → UpperSet W) : Formula α → W → Prop
| .atom a, w   => w ∈ (V a).carrier
...
| .neg φ, w    => ∀ v, w ≤ v → v ∈ Norm.carrier → ¬ Forces Norm V φ v
...
Notice that intuitionistic frames map directly to `Set.univ`. 
-/
def MeetsCondition (sys : LogicType) {W : Type u} [Preorder W] (Norm : UpperSet W) : Prop :=
  match sys with
  | .minimal => True
  | .intuitionistic => Norm.carrier = Set.univ
  | .classical => Norm.carrier = Set.univ ∧ ∀ w v : W, w ≤ v → w = v
...
theorem soundness {α : Type u} {sys : LogicType} {Γ : List (Formula α)} {φ : Formula α}
  ...
  | neg_j h_sys _ _ ih1 ih2 =>
    intro w hΓ v hwv h_norm hφ
    have h_ctx (χ : Formula α) (hχ : χ ∈ φ :: Γ) : Forces Norm V χ v := by
      cases hχ with
      | head => exact hφ
      | tail _ h_tail => exact forces_monotone Norm V χ hwv (hΓ χ h_tail)
  ...
  | neg_i h_sys _ _ ih1 ih2 =>
    intro w hΓ
    subst h_sys
    unfold MeetsCondition at hsys
    have h_norm_top : Norm.carrier = Set.univ := hsys
    have h_not_phi := ih2 w hΓ
    have h_phi := ih1 w hΓ
    have w_in_norm : w ∈ Norm.carrier := by
      rw [h_norm_top]
  ...
  | neg_k h_sys _ _ ih1 ih2 =>
    ...
    have h_ctx (χ : Formula α) (hχ : χ ∈ ~φ :: Γ) : Forces Norm V χ w := by
      cases hχ with
      | head => 
        intro v hwv _
        have hwv_eq : v = w := (h_discrete w v hwv).symm
        subst hwv_eq
        exact h_not_phi
      | tail _ h_tail => exact hΓ χ h_tail
