...
theorem worldShift_zero :
    worldShift (0 : Duration) = Homeomorph.refl World := by
...
theorem worldShift_add (d₁ d₂ : Duration) :
    (worldShift d₁).trans (worldShift d₂) = worldShift (d₂ + d₁) := by
...
For a homeomorphism `e : World ≃ₜ World`, preimage under `e` gives a functor
`Opens World ⥤ Opens World` (contravariantly in the map). We use the preimage
under `worldShift (-d)` so that the *image* under `worldShift d` is captured:
the image of `U` under shift-by-`d` equals the preimage of `U` under shift-by-`(-d)`.

**API note**: Uses `Opens.comap` from `Mathlib.Topology.Opens`. If the exact
API differs in your Mathlib version, the essential content is the functor
induced by the homeomorphism on the open-set lattice.
-/
def opensShift (d : Duration) : (Opens World)ᵒᵖ ⥤ (Opens World)ᵒᵖ :=
  (Opens.comap ⟨worldShift (-d), (worldShift (-d)).continuous⟩).op
