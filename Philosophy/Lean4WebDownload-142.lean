...
lemma foldl_is_const {V : Type} [DecidableEq V] (l : List V) (dv : V → VPoly V) (F : VPoly V)
  ...
  | cons hd tl ih =>
    ...
    apply ih
    · exact ⟨hacc, ⟨hF hd (List.mem_cons_self _ _),
        hdv hd (List.mem_cons_self _ _)⟩⟩
...
theorem taylor_coeff_const (q : ℤ) (k : ℕ) :
  ...
  | n + 2 =>
    ...
    simp [lie_iter_const_eval_succ_succ, h1, h2]
...
lemma MvPolynomial_pderiv_X {V : Type} [DecidableEq V] (v w : V) :
  ...
  simp only [MvPolynomial.pderiv_X, @eq_comm V w v]
...
lemma toMvPolynomial_pderiv_rename {V W : Type} [DecidableEq V] [DecidableEq W]
  ...
  induction P with
  | C z =>
    simp only [VPoly.pderiv, VPoly.rename, VPoly.toMvPolynomial, MvPolynomial.pderiv_C]
  | X x =>
    simp only [VPoly.toMvPolynomial, VPoly.rename, VPoly.pderiv]
    by_cases h : x = v
    · subst h
      simp [MvPolynomial.pderiv_X, VPoly.toMvPolynomial]
    · have hf_ne : f x ≠ f v := fun heq => h (hf heq)
      have hv_ne : f v ≠ f x := Ne.symm hf_ne
      simp [MvPolynomial.pderiv_X, hv_ne, h, VPoly.toMvPolynomial]
  | add p q ihp ihq =>
...
lemma toMvPolynomial_lieDeriv_rename_inl (P : VPoly W₁) :
  ...
  have h_none : MvPolynomial.pderiv none
      (P.rename (some ∘ Sum.inl)).toMvPolynomial = 0 := by
    apply toMvPolynomial_pderiv_rename_ne; intro v contra; cases contra
  have h_inr : ∀ y, MvPolynomial.pderiv (some (Sum.inr y))
      (P.rename (some ∘ Sum.inl)).toMvPolynomial = 0 := by
    intro y; apply toMvPolynomial_pderiv_rename_ne
    intro v contra; injection contra with h; cases h
  have h_inr_sum : (∑ x : W₂, MvPolynomial.pderiv (some (Sum.inr x))
      (P.rename (some ∘ Sum.inl)).toMvPolynomial *
      ((add ar₁ ar₂).deriv (some (Sum.inr x))).toMvPolynomial) = 0 := by
    apply Finset.sum_eq_zero; intro x _; rw [h_inr x, zero_mul]
  have h_inl : ∀ x, MvPolynomial.pderiv (some (Sum.inl x))
      (P.rename (some ∘ Sum.inl)).toMvPolynomial =
      ((P.pderiv x).rename (some ∘ Sum.inl)).toMvPolynomial := by
    intro x; exact toMvPolynomial_pderiv_rename (some ∘ Sum.inl)
  ...
  have h_inl_deriv : ∀ x, ((add ar₁ ar₂).deriv (some (Sum.inl x))).toMvPolynomial =
      ((ar₁.deriv x).rename (some ∘ Sum.inl)).toMvPolynomial := by
    intro x; rfl
...
lemma toMvPolynomial_lieDeriv_rename_inr (P : VPoly W₂) :
  ...
  rw [sum_option_eq, sum_sum_eq]
  have h_none : MvPolynomial.pderiv none
      (P.rename (some ∘ Sum.inr)).toMvPolynomial = 0 := by
    apply toMvPolynomial_pderiv_rename_ne; intro v contra; cases contra
  have h_inl : ∀ x, MvPolynomial.pderiv (some (Sum.inl x))
      (P.rename (some ∘ Sum.inr)).toMvPolynomial = 0 := by
    intro x; apply toMvPolynomial_pderiv_rename_ne
    intro v contra; injection contra with h; cases h
  have h_inl_sum : (∑ x : W₁, MvPolynomial.pderiv (some (Sum.inl x))
      (P.rename (some ∘ Sum.inr)).toMvPolynomial *
      ((add ar₁ ar₂).deriv (some (Sum.inl x))).toMvPolynomial) = 0 := by
    apply Finset.sum_eq_zero; intro x _; rw [h_inl x, zero_mul]
  have h_inr : ∀ y, MvPolynomial.pderiv (some (Sum.inr y))
      (P.rename (some ∘ Sum.inr)).toMvPolynomial =
      ((P.pderiv y).rename (some ∘ Sum.inr)).toMvPolynomial := by
    intro y; exact toMvPolynomial_pderiv_rename (some ∘ Sum.inr)
  ...
  have h_inr_deriv : ∀ y, ((add ar₁ ar₂).deriv (some (Sum.inr y))).toMvPolynomial =
      ((ar₂.deriv y).rename (some ∘ Sum.inr)).toMvPolynomial := by
    intro y; rfl
...
lemma toMvPolynomial_lieIter_add (k : ℕ) :
  ...
  | succ k ih =>
    ...
    have h_eq : (lieIter (add ar₁ ar₂) (k + 1)).toMvPolynomial =
      (VPoly.add ((lieIter ar₁ (k + 1)).rename (some ∘ Sum.inl))
        ((lieIter ar₂ (k + 1)).rename (some ∘ Sum.inr))).toMvPolynomial := by
