...
/-
. Logic.lean (The Metaphysical Deduction)
We must define pure judgments independent of time or space.

Lean 4 Tool: Inductive types and basic Propositional/Predicate logic.

Deep Learning Equiv: The symbolic computation graph before execution.
-/

-- The pure logical form of "If A then B" (Hypothetical Judgment)
-- This has no concept of Time or Space yet.
inductive LogicalForm (α : Type) where
  | categorical : α → α → LogicalForm α
  | hypothetical : LogicalForm α → LogicalForm α → LogicalForm α
  | disjunctive : List (LogicalForm α) → LogicalForm α
