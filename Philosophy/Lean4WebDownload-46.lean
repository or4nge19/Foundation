...
class TemporalManifold (n : ℕ) (Time : Type*) [AddCommGroup Time] [TopologicalSpace Time]    
    (World : Type*) [TopologicalSpace World] [SensibleManifold n World] [VAdd Time World] : Type where    
...
def timeTranslation  
  ...
  [TopologicalSpace World] [VAdd Time World] [ContinuousVAdd Time World] (t : Time) :  
  World ≃ₜ World where  
  toFun := fun x => t +ᵥ x  
  invFun := fun x => (-t) +ᵥ x  
  left_inv := fun x => by simp; 
  right_inv := fun x => by simp [vadd_neg_vadd]  
...
def IsObjectivelyValid  
  {n : ℕ}  
  {World : Type*} [TopologicalSpace World] [SensibleManifold n World]  
  {Time : Type*} [AddCommGroup Time] [TopologicalSpace Time] [VAdd Time World]  
  ...
  ∀ (t : Time) (U : Opens World),  
    let shifted_U : Opens World := (timeTranslation t).isOpenMap.functor.obj U  
