...
/-!
# Kripke Semantics for Minimal, Intuitionistic, and Classical Logic

This module formalizes Kripke semantics for propositional logic as outlined in 
Chapters 2 and 3 of Sergio Galvan's "Non Contraddizione e Terzo Escluso".

## Architectural Design
To align with `mathlib`'s principles of modularity and order theory:
- We use a single `Preorder W` to represent the worlds.
...
theorem forces_monotone {α W : Type u} [Preorder W] (Norm : UpperSet W) (V : α → UpperSet W)
  ...
  | atom a => 
    -- Relies on mathlib's UpperSet native closure property
    exact (V a).upper' hwv h
  | and φ ψ ih1 ih2 => 
    exact ⟨ih1 hwv h.1, ih2 hwv h.2⟩
  | or φ ψ ih1 ih2 => 
    cases h with
...
- `intuitionistic`: `Norm = ⊤` (every world is normal).
- `classical`: `Norm = ⊤` and the frame is discrete (collapsing to Boolean valuations).
-/
...
theorem soundness {α : Type u} {sys : LogicType} {Γ : List (Formula α)} {φ : Formula α}
  ...
  | neg_j h_sys _ _ ih1 ih2 =>
    ...
    have h_ctx : ∀ χ ∈ φ :: Γ, Forces Norm V χ v := by
      intro χ hχ
      cases hχ with
      | head => exact hφ
    ...
    -- Apply the contradiction locally at normal world `v`
    exact h_not_psi v (le_refl v) h_norm h_psi
  
  | neg_i h_sys _ _ ih1 ih2 =>
    -- Intuitionistic constraint enforces that all worlds are Normal
    subst h_sys
  ...
  | neg_k h_sys _ _ ih1 ih2 =>
    -- Classical constraint enforces Normality and discrete preorders
    subst h_sys
    rcases hsys with ⟨h_norm_top, h_discrete⟩
    
    -- Classical meta-logic is needed to resolve classical boolean truth
    open Classical in
    by_contra h_not_phi
    
    have h_not_phi_force : Forces Norm V (~φ) w := by
      intro v hwv _
      have hwv_eq : w = v := h_discrete w v hwv
      subst hwv_eq
      exact h_not_phi
      
    have h_ctx : ∀ χ ∈ ~φ :: Γ, Forces Norm V χ w := by
      intro χ hχ
      cases hχ with
      | head => exact h_not_phi_force
