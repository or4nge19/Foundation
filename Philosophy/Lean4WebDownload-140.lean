...
lemma foldl_isConst {V} (l : List V) [DecidableEq V] (dv : V → Poly V) (F : Poly V) (acc : Poly V)
  ...
  | cons hd tl ih =>
    ...
    apply ih
    · dsimp[Poly.IsConst]; exact ⟨hacc, ⟨hF hd List.mem_cons_self, hdv hd List.mem_cons_self⟩⟩
...
lemma lieIter_const_isConst (q : ℤ) (k : ℕ) :
  ...
  | zero =>
    ...
    · intro v; simp [Poly.pderiv, Poly.IsConst]
  | succ k ih =>
...
theorem approxSum_const (q : ℤ) (n : ℕ) (hn : 1 ≤ n) :
  ...
  | succ n ih =>
    ...
    · subst hn_zero
      rw [approxSum_as_sum]
      have : List.range (1 + 1) = [0, 1] := rfl
      rw [this]
      simp [taylorCoeff_const]
    · have h_pos : 1 ≤ n := by omega
      have ih' := ih h_pos
      rw[approxSum_as_sum] at ih' ⊢
      have : List.range (n + 1 + 1) = List.range (n + 1) ++ [n + 1] := List.range_succ
      ...
      have h_coeff : taylorCoeff (const q) (n + 1) = 0 := by
        ...
        simp_rw [h2]; exact Rat.nonneg_antisymm rfl rfl
      rw [h_coeff, add_zero]
...
axiom taylorCoeff_add (k : ℕ) :
    taylorCoeff (add ar₁ ar₂) k = taylorCoeff ar₁ k + taylorCoeff ar₂ k
