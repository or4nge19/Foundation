...
def IsLove (a : AffectWithCause Mode) : Prop :=
  isJoy Mode Perfection a.transition

def IsHate (a : AffectWithCause Mode) : Prop :=
...
def IsFavor {U V : Mode} (a : AffectWithCause Mode) (benefit : U ⟶ V) : Prop :=
  IsLove Mode Perfection a ∧ isJoy Mode Perfection benefit

/-- Indignation: hate toward one who has done evil. -/
def IsIndignation {U V : Mode} (a : AffectWithCause Mode) (harm : U ⟶ V) : Prop :=
...
theorem e5p36_amor_dei (X G : Mode)
  ...
  unfold IsBlessed potentia at hX hG
  rw [hX, hG]
