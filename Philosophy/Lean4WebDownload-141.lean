...
def toMvPolynomial : VPoly V → MvPolynomial V ℚ
...
lemma toMvPolynomial_pderiv [DecidableEq V] (p : VPoly V) (v : V) :
  ...
  induction p <;> simp [toMvPolynomial, pderiv, *]
  case X x => split_ifs <;> simp [*]
...
theorem lie_deriv_eval {V : Type} [Fintype V][DecidableEq V]
  ...
  suffices h : ∀ (acc : VPoly V) (l : List V),
    ...
    rw [h₀]
    exact (Finset.sum_toList Finset.univ).symm ▸ rfl
  intro acc l
...
theorem lie_iter_const_eval_one (q : ℤ) :
  ...
  have eq : (lieDeriv (const q).deriv (VPoly.X ())).eval (const q).init =
    ∑ v : Unit, ((VPoly.X ()).pderiv v).eval (const q).init * ((const q).deriv v).eval (const q).init :=
    lie_deriv_eval (V := Unit) (R := ℚ) _ _ _
  rw [eq]
  simp [const]

def VPoly.IsConst {V} : VPoly V → Prop
  | .C _ => True
  | .X _ => False
...
lemma VPoly.is_const_pderiv {V}[DecidableEq V] (p : VPoly V) (v : V) :
...
lemma VPoly.eval_pderiv_of_is_const {V} [DecidableEq V] {R : Type} [CommRing R]
...
lemma foldl_is_const {V} (l : List V)[DecidableEq V] (dv : V → VPoly V) (F : VPoly V) (acc : VPoly V)
    (hacc : acc.IsConst)
  ...
  | cons hd tl ih =>
    ...
    apply ih
    · dsimp [VPoly.IsConst]; exact ⟨hacc, ⟨hF hd (List.mem_cons_self _ _), hdv hd (List.mem_cons_self _ _)⟩⟩
    · intro v hv; exact hdv v (List.mem_cons_of_mem _ hv)
    · intro v hv; exact hF v (List.mem_cons_of_mem _ hv)

lemma lie_deriv_is_const {V}[Fintype V] [DecidableEq V] (dv : V → VPoly V) (F : VPoly V)
...
lemma lie_iter_const_is_const (q : ℤ) (k : ℕ) :
  ...
  | zero =>
    ...
    · intro v; dsimp [VPoly.pderiv]; split_ifs <;> exact trivial
  | succ k ih =>
...
theorem lie_iter_const_eval_succ_succ (q : ℤ) (k : ℕ) :
  ...
  rw[lie_deriv_eval (V := Unit) (R := ℚ), Fintype.sum_unique, lie_iter_const_pderiv_eval_zero q k]
  ring
...
theorem taylor_coeff_const (q : ℤ) (k : ℕ) :
  ...
  | n + 2 => simp [lie_iter_const_eval_succ_succ]
...
lemma sum_option_eq {α M} [Fintype α][DecidableEq α] [AddCommMonoid M] (f : Option α → M) :
    ∑ x : Option α, f x = f none + ∑ x : α, f (some x) := by
  classical
  have : (Finset.univ : Finset (Option α)) = insert none (Finset.image some Finset.univ) := by ext x; cases x <;> simp
...
lemma sum_sum_eq {α β M} [Fintype α] [Fintype β] [DecidableEq α] [DecidableEq β] [AddCommMonoid M] (f : α ⊕ β → M) :
    ∑ x : α ⊕ β, f x = (∑ x : α, f (Sum.inl x)) + (∑ x : β, f (Sum.inr x)) := by
  classical
  have : (Finset.univ : Finset (α ⊕ β)) = (Finset.image Sum.inl Finset.univ) ∪ (Finset.image Sum.inr Finset.univ) := by
    ext x; cases x <;> simp
...
lemma MvPolynomial_pderiv_X {V}[DecidableEq V] (v w : V) :
    MvPolynomial.pderiv v (MvPolynomial.X w : MvPolynomial V ℚ) = if w = v then 1 else 0 := by
  have h : MvPolynomial.pderiv v (MvPolynomial.X w : MvPolynomial V ℚ) = (VPoly.pderiv v (VPoly.X w)).toMvPolynomial := by
    rw [VPoly.toMvPolynomial_pderiv]
    rfl
  rw[h]
  dsimp [VPoly.pderiv]
  split_ifs
...
lemma toMvPolynomial_lieDeriv {V} [Fintype V][DecidableEq V] (dv : V → VPoly V) (F : VPoly V) :
    (lieDeriv dv F).toMvPolynomial = ∑ v : V, MvPolynomial.pderiv v F.toMvPolynomial * (dv v).toMvPolynomial := by
  unfold lieDeriv
  set g : V → MvPolynomial V ℚ := fun v => MvPolynomial.pderiv v F.toMvPolynomial * (dv v).toMvPolynomial
  suffices h : ∀ (acc : VPoly V) (l : List V),
...
lemma toMvPolynomial_lieDeriv_add {V} [Fintype V] [DecidableEq V] (dv : V → VPoly V) (F G : VPoly V) :
    (lieDeriv dv (VPoly.add F G)).toMvPolynomial = (lieDeriv dv F).toMvPolynomial + (lieDeriv dv G).toMvPolynomial := by
  rw[toMvPolynomial_lieDeriv, toMvPolynomial_lieDeriv, toMvPolynomial_lieDeriv, VPoly.toMvPolynomial]
  simp [map_add, add_mul, Finset.sum_add_distrib]

lemma MvPolynomial_lieDeriv_congr {V} [Fintype V] [DecidableEq V] (dv : V → VPoly V) (F G : VPoly V) (h : F.toMvPolynomial = G.toMvPolynomial) :
    (lieDeriv dv F).toMvPolynomial = (lieDeriv dv G).toMvPolynomial := by
  rw[toMvPolynomial_lieDeriv, toMvPolynomial_lieDeriv]
  simp [h]

lemma toMvPolynomial_lieDeriv_X {V} [Fintype V] [DecidableEq V] (dv : V → VPoly V) (v : V) :
    (lieDeriv dv (VPoly.X v)).toMvPolynomial = (dv v).toMvPolynomial := by
  rw[toMvPolynomial_lieDeriv]
  have h1 : ∑ x : V, MvPolynomial.pderiv x (VPoly.X v).toMvPolynomial * (dv x).toMvPolynomial = MvPolynomial.pderiv v (VPoly.X v).toMvPolynomial * (dv v).toMvPolynomial := by
...
lemma toMvPolynomial_pderiv_rename_ne {V W} [DecidableEq V] [DecidableEq W] (f : V → W) (P : VPoly V) (w : W) (hw : ∀ v, f v ≠ w) :
    MvPolynomial.pderiv w (P.rename f).toMvPolynomial = 0 := by
  induction P with
  | C z => simp[VPoly.toMvPolynomial, VPoly.rename]
  | X x =>
    dsimp[VPoly.toMvPolynomial, VPoly.rename]
...
lemma toMvPolynomial_pderiv_rename {V W}[DecidableEq V] [DecidableEq W] (f : V → W) (hf : Function.Injective f) (P : VPoly V) (v : V) :
    MvPolynomial.pderiv (f v) (P.rename f).toMvPolynomial = (P.pderiv v).rename f |>.toMvPolynomial := by
  induction P with
  | C z => simp[VPoly.toMvPolynomial, VPoly.rename, VPoly.pderiv]
  | X x =>
    dsimp [VPoly.toMvPolynomial, VPoly.rename, VPoly.pderiv]
    have : f x = f v ↔ x = v := ⟨hf, fun h => h ▸ rfl⟩
    by_cases h : x = v
    · simp[h, MvPolynomial_pderiv_X]
    · have hf_ne : f x ≠ f v := mt hf h
      simp [h, hf_ne, MvPolynomial_pderiv_X]
      rfl
  | add p q ihp ihq => simp[VPoly.toMvPolynomial, VPoly.rename, VPoly.pderiv, ihp, ihq]
  | mul p q ihp ihq => simp[VPoly.toMvPolynomial, VPoly.rename, VPoly.pderiv, ihp, ihq]
  | neg p ihp => simp[VPoly.toMvPolynomial, VPoly.rename, VPoly.pderiv, ihp]

lemma toMvPolynomial_rename_lieDeriv {V W} [Fintype V] [DecidableEq V] (f : V → W) (dv : V → VPoly V) (P : VPoly V) :
    ((lieDeriv dv P).rename f).toMvPolynomial = ∑ x : V, (P.pderiv x).rename f |>.toMvPolynomial * (dv x).rename f |>.toMvPolynomial := by
  unfold lieDeriv
  set g : V → MvPolynomial W ℚ := fun x => (P.pderiv x).rename f |>.toMvPolynomial * (dv x).rename f |>.toMvPolynomial
  suffices h : ∀ (acc : VPoly V) (l : List V),
      ((l.foldl (fun a v => a + P.pderiv v * dv v) acc).rename f).toMvPolynomial =
        (acc.rename f).toMvPolynomial + (l.map g).sum by
    have h₀ := h (VPoly.C 0) (Finset.univ : Finset V).toList
    simp only[VPoly.rename, VPoly.toMvPolynomial, Int.cast_zero, map_zero, zero_add] at h₀
    rw [h₀]
    exact (Finset.sum_toList Finset.univ).symm ▸ rfl
  intro acc l
  induction l generalizing acc with
...
lemma toMvPolynomial_lieDeriv_rename_inl (P : VPoly V₁) :
    (lieDeriv (add ar₁ ar₂).deriv (P.rename (some ∘ Sum.inl))).toMvPolynomial =
    (lieDeriv ar₁.deriv P).rename (some ∘ Sum.inl) |>.toMvPolynomial := by
  rw[toMvPolynomial_lieDeriv, toMvPolynomial_rename_lieDeriv]
  rw[sum_option_eq, sum_sum_eq]
  have h_none : MvPolynomial.pderiv none (P.rename (some ∘ Sum.inl)).toMvPolynomial = 0 := by
    apply toMvPolynomial_pderiv_rename_ne; intro v contra; cases contra
  have h_inr : ∀ y, MvPolynomial.pderiv (some (Sum.inr y)) (P.rename (some ∘ Sum.inl)).toMvPolynomial = 0 := by
    intro y; apply toMvPolynomial_pderiv_rename_ne; intro v contra; injection contra with h; cases h
  have h_inr_sum : (∑ x : V₂, MvPolynomial.pderiv (some (Sum.inr x)) (P.rename (some ∘ Sum.inl)).toMvPolynomial * ((add ar₁ ar₂).deriv (some (Sum.inr x))).toMvPolynomial) = 0 := by
    apply Finset.sum_eq_zero; intro x _; rw [h_inr x, zero_mul]
  have h_inl : ∀ x, MvPolynomial.pderiv (some (Sum.inl x)) (P.rename (some ∘ Sum.inl)).toMvPolynomial = (P.pderiv x).rename (some ∘ Sum.inl) |>.toMvPolynomial := by
    intro x; exact toMvPolynomial_pderiv_rename (some ∘ Sum.inl) (by intro a b h; injection h with h; injection h with h; exact h) P x
  have h_inl_deriv : ∀ x, ((add ar₁ ar₂).deriv (some (Sum.inl x))).toMvPolynomial = (ar₁.deriv x).rename (some ∘ Sum.inl) |>.toMvPolynomial := by
    intro x; rfl
...
lemma toMvPolynomial_lieDeriv_rename_inr (P : VPoly V₂) :
    (lieDeriv (add ar₁ ar₂).deriv (P.rename (some ∘ Sum.inr))).toMvPolynomial =
    (lieDeriv ar₂.deriv P).rename (some ∘ Sum.inr) |>.toMvPolynomial := by
  rw[toMvPolynomial_lieDeriv, toMvPolynomial_rename_lieDeriv]
  rw [sum_option_eq, sum_sum_eq]
  have h_none : MvPolynomial.pderiv none (P.rename (some ∘ Sum.inr)).toMvPolynomial = 0 := by
    apply toMvPolynomial_pderiv_rename_ne; intro v contra; cases contra
  have h_inl : ∀ x, MvPolynomial.pderiv (some (Sum.inl x)) (P.rename (some ∘ Sum.inr)).toMvPolynomial = 0 := by
    intro x; apply toMvPolynomial_pderiv_rename_ne; intro v contra; injection contra with h; cases h
  have h_inl_sum : (∑ x : V₁, MvPolynomial.pderiv (some (Sum.inl x)) (P.rename (some ∘ Sum.inr)).toMvPolynomial * ((add ar₁ ar₂).deriv (some (Sum.inl x))).toMvPolynomial) = 0 := by
    apply Finset.sum_eq_zero; intro x _; rw[h_inl x, zero_mul]
  have h_inr : ∀ y, MvPolynomial.pderiv (some (Sum.inr y)) (P.rename (some ∘ Sum.inr)).toMvPolynomial = (P.pderiv y).rename (some ∘ Sum.inr) |>.toMvPolynomial := by
    intro y; exact toMvPolynomial_pderiv_rename (some ∘ Sum.inr) (by intro a b h; injection h with h; injection h with h; exact h) P y
  have h_inr_deriv : ∀ y, ((add ar₁ ar₂).deriv (some (Sum.inr y))).toMvPolynomial = (ar₂.deriv y).rename (some ∘ Sum.inr) |>.toMvPolynomial := by
    intro y; rfl
...
lemma toMvPolynomial_lieIter_add (k : ℕ) :
    ...
    (lieIter ar₁ (k + 1)).rename (some ∘ Sum.inl) |>.toMvPolynomial +
    (lieIter ar₂ (k + 1)).rename (some ∘ Sum.inr) |>.toMvPolynomial := by
  induction k with
  | zero =>
    dsimp [lieIter, add]
    rw[toMvPolynomial_lieDeriv_X]
    simp [VPoly.toMvPolynomial]
  | succ k ih =>
    have h_step : lieIter (add ar₁ ar₂) (k + 2) = lieDeriv (add ar₁ ar₂).deriv (lieIter (add ar₁ ar₂) (k + 1)) := rfl
    rw [h_step]
    have h_eq : (lieIter (add ar₁ ar₂) (k + 1)).toMvPolynomial =
      (VPoly.add ((lieIter ar₁ (k + 1)).rename (some ∘ Sum.inl)) ((lieIter ar₂ (k + 1)).rename (some ∘ Sum.inr))).toMvPolynomial := by
...
lemma taylorCoeff_add_succ (k : ℕ) :
    taylorCoeff (add ar₁ ar₂) (k + 1) = taylorCoeff ar₁ (k + 1) + taylorCoeff ar₂ (k + 1) := by
  unfold taylorCoeff
  ...
  have h_init1 : (add ar₁ ar₂).init ∘ some ∘ Sum.inl = ar₁.init := rfl
  have h_init2 : (add ar₁ ar₂).init ∘ some ∘ Sum.inr = ar₂.init := rfl
  rw[h_init1, h_init2]
  ring

/-- Syntactic separation and sum linearity rigorously resolves the structural equivalence mapping identically without axioms. -/
